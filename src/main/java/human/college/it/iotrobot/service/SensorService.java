...
import human.college.it.iotrobot.exception.ImageConvertToByteArrayException;
...
public class SensorService {
    ...
    public List<LocalDate> getAvailableDates() {
        return sensorRepository.findDistinctDateOrderByDateDesc();
    }
     ...
     * @return 対象日のセンサーデータ
     */
    public List<Sensor> getTargetDateSensors(LocalDate targetDate) {
        LocalDateTime targetDateStart = targetDate.atStartOfDay();
        LocalDateTime targetDateEnd = targetDate.plusDays(1).atStartOfDay().minusNanos(1);
    ...
    public byte[] generateGraphImage(LocalDate date) {
        try {
            // 対象日のセンサーデータを取得
            List<Sensor> sensors = getTargetDateSensors(date);
     ...
     * 温度データから折れ線グラフを作成する
     */
    private JFreeChart createTemperatureChart(List<Sensor> sensors, LocalDate date) {
        // TimeSeries作成
    ...
    private byte[] chartToByteArray(JFreeChart chart, int width, int height) {
        ...
        try {
            ...
            throw new ImageConvertToByteArrayException("グラフ画像のバイト列変換に失敗しました", e);
        }
    }
}
